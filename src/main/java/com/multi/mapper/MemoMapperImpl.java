...
public class MemoMapperImpl implements MemoMapper {
	...
	public int insertMemo(MemoVO memo) {
		//먼저 글번호no에 넣어줄 값을 sequence컬렉션에서 가져오되, count값을 1씩 증가시킨뒤 가져온다
		Update update=new Update().inc("count", 1);//count값을 1씩 증가시킨다. {$inc:{count:1}}
		...
		log.info("svo==="+svo);
		memo.setNo(svo.getCount());//시퀀스에 해당하는 count값을 글번호로 설정하자		
